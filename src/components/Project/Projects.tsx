...
const Projects = ({ preview = false }: { preview: boolean }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {!preview && (
                <div className="flex flex-col gap-4 mx-4 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-1">
                        <h1 className="text-2xl font-bold text-slate-900">Projects</h1>
                        <p className="text-sm text-slate-600">
                            {view === 'cards' ? 'Visual overview of all projects' : 'Detailed list of all projects'}
                            {searchQuery && ` • ${filteredProjects.length} results found`}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-blue-700">
                                Found <span className="font-semibold">{filteredProjects.length}</span> project{filteredProjects.length !== 1 ? 's' : ''} matching "<span className="font-semibold">{searchQuery}</span>"
                            </span>
                        </div>
                        <button
                            onClick={() => setSearchQuery('')}
            ...
            {/* Error State */}
            {error && !loading && (
                <ProjectTableError error={error} loadMyProjects={loadMyProjects} />
            )}

            {/* Empty State */}
            {!loading && !error && filteredProjects.length === 0 && (
                <ProjectEmptyState searchQuery={searchQuery} />
            )}
            ...
            {!loading && !error && filteredProjects.length > 0 && (
                <>
                    {view === 'cards' ? (
                        <ProjectCards projects={filteredProjects} preview={preview} />
                    ) : (
